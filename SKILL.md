...
- For subdirectory/monorepo context, **the content always lives in nested `AGENTS.md` files, never in nested `CLAUDE.md`**. AGENTS.md's nested, closest-file-wins model is honored natively across 20+ agents (Codex, Cursor, Copilot, Gemini CLI, Claude Code, and others), so it's the portable choice regardless of which coding agent ends up being used. When Claude Code is a confirmed target agent, each nested `AGENTS.md` gets a sibling `CLAUDE.md` that does nothing but `@AGENTS.md` — Claude Code's context loading is CLAUDE.md-centric and won't reliably pick up a bare AGENTS.md without an opt-in setting enabled first, so the companion file is what makes the docs actually load. Content stays in AGENTS.md either way.
