...
- **`plan-module` and `execute-plan` could not be uploaded to Claude.ai or Claude Desktop.** Both
  ...
  4.2.0 shipped this bug in its published `.skill` assets, so upgrading is the only way to get those
  two skills into Claude.ai or Claude Desktop. Nothing else changed — the skills behave identically
  on Claude Code, Codex, Cursor and every other agent, which never rejected the old description.
