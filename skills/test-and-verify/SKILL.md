...
description: Runs a project's tests, reads the real output, and fixes what fails — a bounded diagnose-fix-rerun loop that stops after three remediation attempts rather than guessing on. Targets the suite relevant to the work by default, and adds the type-checker and linter when it is verifying a phase gate. Reports a clear pass or fail verdict with the exact command it ran, and never edits a plan file — the caller marks the boxes. Use this skill when the user says things like "run the tests", "run the test suite", "verify this", "check the tests pass", "fix the failing tests", "why is this test failing", or when `execute-plan` needs a task or a `Task X.V` gate verified. Not for writing a feature or its tests from a plan — use execute-plan. Not for cutting a module into phases — use plan-module. Not for setting up a project's docs or its first test runner — use scaffold and execute-plan.
---
